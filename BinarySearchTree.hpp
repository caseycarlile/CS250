...
public:
    ...
    */
    ...
    void Delete( const TK& key )
    {
		...
		{
			Node<TK, TD> *temp = node->ptrRight;
			Node<TK, TD> *next = node;
			Node<TK, TD> *nextParent = node;
			...
			if (next != node->ptrRight)
			{
				nextParent->ptrLeft = next->ptrRight;
				nextParent->ptrRight = next->ptrRight;
			}
			...
			else
			{
				...
				parent->ptrRight = next;
			}
			
			next->ptrLeft = node->ptrLeft;
		}
