...
{
	int max_capacity = 0;
	cout << "Enter max capacity for the airplane: ";
	cin >> max_capacity;
...
bool AirTravelSimulator::LineUp()
{
	...
	Traveller* person = m_pplManager.GetTraveller(random);

	m_airport.LineUp(person);

	m_ptrTravellers.PushBack(person);

	person->state = LINEUP;

	DisplayMessage(person, LINEUP);

	return (m_airport.GetMaxCapacity() == m_airport.WaitingCount());
...
bool AirTravelSimulator::Board()
{
	...
	Traveller* person = m_airport.NextInLine();

	m_airplane.Board(person);

	person->state = BOARDING;

	DisplayMessage(person, BOARDING);
...
{
	Traveller* person = m_airplane.Disembark();

	person->state = FINISHED;

	DisplayMessage(person, DISEMBARKING);
